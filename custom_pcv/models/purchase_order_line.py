...
class PurchaseOrderLine(models.Model):
    ...
    stand_name      = fields.Char('Stand name', store=True, compute=_get_sale_stand_name)

    @api.depends('create_date')
    def _get_sale_stand_number(self):
        for record in self:
            record.event_id = record.sale_order_id.stand_number
    stand_number    = fields.Char('Stand number', store=True, compute=_get_sale_stand_number)